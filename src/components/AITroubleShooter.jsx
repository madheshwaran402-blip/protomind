...
function AITroubleshooter({ idea, components }) {
  ...
  const TABS = [{ id: 'diagnosis', label: 'Diagnosis' }, { id: 'steps', label: 'Fix Steps' }, { id: 'history', label: 'History' }]

  return (
    <div className="space-y-4">
      <div className="space-y-3">
        <textarea value={problem} onChange={function(e) { setProblem(e.target.value) }}
          placeholder="Describe your problem e.g. My sensor always shows 0"
        ...
        <div className="flex flex-wrap gap-1">
          {COMMON_PROBLEMS.slice(0, 4).map(function(p, i) {
            return (
        ...
        <button onClick={function() { handleTroubleshoot() }} disabled={loading || !problem.trim() || components.length === 0}
      ...
      {loading && (
        <div className="flex items-center justify-center py-8 gap-3 bg-[#13131f] border border-[#2e2e4e] rounded-xl">
          ...
          <p className="text-slate-400 text-sm">Diagnosing...</p>
        </div>
      )}
              ...
              {(result.rootCauses || []).map(function(cause, i) {
                return (
                  <div key={i} className="bg-[#13131f] border border-[#2e2e4e] rounded-xl p-4">
                    <p className="text-white text-sm font-semibold">{cause.cause}</p>
                    <p className="text-slate-400 text-xs">{cause.explanation}</p>
                  </div>
                )
              })}
                ...
                <div className="bg-green-950 border border-green-800 rounded-xl p-4">
                  ...
                  <ul className="space-y-1">
                    {result.quickFixes.map(function(fix, i) {
                      return <li key={i} className="text-slate-300 text-xs">{i+1}. {fix}</li>
                    })}
                  </ul>
                </div>
              )}
              ...
              {(result.steps || []).map(function(step, i) {
                ...
                return (
                  <div key={i} onClick={function() { toggleStep(i) }}
                    ...
                    <p className={"text-sm " + (done ? "line-through text-slate-500" : "text-white")}>{step.action}</p>
              ...
              {history.map(function(session, i) {
                return (
                  <div key={i} onClick={function() { setResult(session); setActiveTab("diagnosis") }}
                    ...
                    <p className="text-slate-500 text-xs">{session.diagnosis}</p>
                  </div>
                )
              })}
            </div>
      ...
      {!result && !loading && history.length === 0 && (
        <div className="text-center py-8 bg-[#13131f] border border-[#2e2e4e] rounded-xl">
          ...
          <p className="text-slate-500 text-sm">Describe any problem to get diagnosis and fixes</p>
        </div>
      )}
    </div>
  )
