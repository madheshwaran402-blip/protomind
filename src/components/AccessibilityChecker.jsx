...
function AccessibilityChecker({ idea, components }) {
  ...
  const scoreColor = result
    ? result.accessibilityScore >= 75 ? '#22c55e'
    : result.accessibilityScore >= 50 ? '#f59e0b'
    : '#ef4444'
    : '#6366f1'
    ...
    <div className="space-y-4">
          ...
          <div className="bg-[#0d0d1a] border border-[#1e1e2e] rounded-2xl p-5 flex items-center gap-5">
            <div className="relative w-20 h-20 shrink-0">
              <svg className="w-full h-full -rotate-90" viewBox="0 0 80 80">
                ...
                <circle cx="40" cy="40" r="35" fill="none" stroke={scoreColor} strokeWidth="6"
                  ...
                  strokeDashoffset={2 * Math.PI * 35 * (1 - result.accessibilityScore / 100)}
                  strokeLinecap="round" />
              </svg>
            ...
            <div>
              ...
              {result.positives && result.positives.length > 0 && (
                <p className="text-green-400 text-xs">{result.positives.length} positive features</p>
              )}
            </div>
          </div>
              ...
              return (
                <button key={tab.id} onClick={function() { setActiveTab(tab.id) }}
                  className={'flex-1 py-2 rounded-lg text-xs font-medium transition ' + (
                    activeTab === tab.id ? 'bg-blue-700 text-white' : 'text-slate-500 hover:text-white'
                  )}>
                  {tab.label}
                </button>
              )
            ...
            <div className="space-y-2">
              {result.positives && result.positives.length > 0 && (
                <div className="bg-green-950 border border-green-800 rounded-xl p-3">
                  ...
                  {result.positives.map(function(p, i) {
                    return <p key={i} className="text-slate-300 text-xs">• {p}</p>
                  })}
                </div>
              )}
              {(result.issues || []).length === 0 ? (
                <p className="text-green-400 text-sm text-center py-4">✅ No major accessibility issues found!</p>
              ) : (
                  ...
                  return (
                    <div key={i} className={'rounded-xl border p-4 ' + style.split(' ').slice(1).join(' ')}>
                      ...
                      <div className="bg-[#0d0d1a] rounded-lg p-2">
                        <p className="text-green-400 text-xs font-semibold">Solution:</p>
                        <p className="text-slate-300 text-xs">{issue.solution}</p>
                      </div>
                    </div>
                  )
                })
              ...
              {(result.improvements || []).map(function(imp, i) {
                return (
                  <div key={i} className="bg-[#13131f] border border-[#2e2e4e] rounded-xl p-4">
                    ...
                    {imp.implementation && (
                      <p className="text-indigo-400 text-xs">💡 How: {imp.implementation}</p>
                    )}
                  </div>
                )
              })}
      ...
      {!result && !loading && (
        <div className="text-center py-8 bg-[#13131f] border border-[#2e2e4e] rounded-xl">
          ...
          <p className="text-slate-500 text-sm">Check your prototype for inclusive design and accessibility issues</p>
        </div>
      )}
    </div>
  )
