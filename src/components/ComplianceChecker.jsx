...
function ComplianceChecker({ idea, components }) {
  ...
  const TABS = [{ id: 'certs', label: '📋 Certifications' }, { id: 'standards', label: '📐 Standards' }, { id: 'risks', label: '⚠️ Risks' }]
    ...
    <div className="space-y-4">
            ...
            {TABS.map(function(tab) {
              return (
                <button key={tab.id} onClick={function() { setActiveTab(tab.id) }}
                  className={'flex-1 py-2 rounded-lg text-xs font-medium transition ' + (activeTab === tab.id ? 'bg-teal-700 text-white' : 'text-slate-500 hover:text-white')}>
                  {tab.label}
                </button>
              )
            })}
                ...
                return (
                  <div key={i} className={'rounded-xl border p-4 ' + (cert.required ? 'bg-red-950 border-red-900' : 'bg-[#13131f] border-[#2e2e4e]')}>
                    ...
                    <div className="flex gap-3 text-xs mt-1">
                      {cert.cost && <span className="text-emerald-400">{cert.cost}</span>}
                      {cert.timeframe && <span className="text-blue-400">{cert.timeframe}</span>}
                    </div>
                  </div>
                )
              })}
                ...
                <div className="bg-red-950 border border-red-900 rounded-xl p-4">
                  ...
                  <ul className="space-y-1">
                    {result.risks.map(function(risk, i) {
                      return <li key={i} className="text-slate-300 text-xs flex gap-2"><span className="text-red-400">•</span>{risk}</li>
                    })}
                  </ul>
                </div>
              )}
      ...
      {!result && !loading && (
        <div className="text-center py-8 bg-[#13131f] border border-[#2e2e4e] rounded-xl">
          ...
          <p className="text-slate-500 text-sm">Check CE, FCC, RoHS and other certifications needed</p>
        </div>
      )}
    </div>
  )
