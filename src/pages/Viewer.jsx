...
<p className="text-xs text-slate-600 mb-3 uppercase tracking-widest font-semibold">
  ⚡ {selectedComponents.length} Components · {20}+ AI Tools — click any to expand
      ...
      {/* ─── TODAY'S TASK + ROADMAP BANNER ─── */}
      {(function() {
        const req = (function() {
          try { return JSON.parse(localStorage.getItem('protomind_current_requirements') || 'null') } catch(e) { return null }
        })()
        const roadmapKey = req ? 'protomind_roadmap_' + btoa(req.idea || '').slice(0,20) : null
        const roadmapData = (function() {
          try { return roadmapKey ? JSON.parse(localStorage.getItem(roadmapKey) || 'null') : null } catch(e) { return null }
        })()
        const completedDays = roadmapData?.completedDays || {}
        const allDays = (roadmapData?.roadmap?.phases || []).flatMap(function(p) { return p.days || [] }).sort(function(a,b){return a.day-b.day})
        const currentDay = allDays.find(function(d) { return !completedDays[d.day] }) || allDays[allDays.length-1]
        ...
        return (
          <div className="mb-4 bg-gradient-to-r from-indigo-950 to-[#0d0d1a] border border-indigo-800 rounded-2xl p-4">
              ...
              <div className="flex-1 min-w-64">
                ...
                {currentDay.tasks && currentDay.tasks.length > 0 && (
                  <div className="mt-2 space-y-1">
                    {currentDay.tasks.slice(0,3).map(function(task, i) {
                      return (
                    ...
                    {currentDay.tasks.length > 3 && (
                      <p className="text-slate-600 text-xs">+{currentDay.tasks.length - 3} more tasks</p>
                    )}
                  </div>
                )}
              </div>
              ...
              <div className="flex flex-col items-end gap-2">
                <div className="flex items-center gap-3">
                  <div className="text-right">
                    <p className="text-indigo-400 font-black text-2xl">{pct}%</p>
                ...
                <a href="/roadmap"
                  className="px-3 py-1.5 bg-indigo-700 hover:bg-indigo-600 text-white rounded-xl text-xs font-bold transition flex items-center gap-1">
                  View Full Roadmap →
                </a>
              </div>
            </div>
