...
const PAGES = [
  ...
  { path:'/hub', icon:'🧭', title:'Nav Hub', desc:'This page', color:'#64748b', tag:'Core' },
]
  ...
  return (
    <div className="min-h-screen bg-[#050510] text-white p-6">
      <div className="max-w-5xl mx-auto">
        ...
        <h2 className="text-lg font-bold text-indigo-400 mb-3">New Features (30-Day Sprint)</h2>
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 mb-8">
          {newPages.map(function(page) {
            return (
              <button key={page.path} onClick={function() { navigate(page.path) }}
                className="p-4 rounded-2xl border text-left hover:scale-105 transition-all duration-200"
                ...
                <span className="inline-block mt-2 text-xs px-2 py-0.5 rounded-full font-bold"
                  style={{ backgroundColor: page.color + '20', color: page.color }}>NEW</span>
              </button>
            )
          })}
        </div>
        ...
        <div className="bg-gradient-to-r from-indigo-950 to-purple-950 border border-indigo-800 rounded-2xl p-6">
          <h3 className="text-white font-bold text-lg mb-3">Quick Actions</h3>
          <div className="flex flex-wrap gap-3">
            {[
              { l: 'Quick Build', p: '/' },
              { l: 'Start with Requirements', p: '/wizard' },
              { l: 'Scan a Component', p: '/protoscan' },
              { l: 'Open Simulator', p: '/simulator2' },
              { l: 'Open IDE', p: '/ide' },
              { l: 'Digital Twin', p: '/digitaltwin' },
              { l: 'View Roadmap', p: '/roadmap' },
            ].map(function(item) {
              return (
